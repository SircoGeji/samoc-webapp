...
context('Save, edit, duplicate and delete acquisition offers', () => {
  ...
  const offerCodeFull1 = `${offerPrefix}acq_${dateString}_${offerCode1}`;
  const offerCodeFullDuration1 = `${offerPrefix}acq_${dateString}_${offerCode1}500_for_3mo`;
  const offerCodeFullDuration2 = `${offerPrefix}acq_${dateString}_${offerCode1}500_for_3mo_v2`;
  ...
  it('should visit offers page and validate synchronization block', () => {
    web_PO.getOffers();
    cy.visit(offersUrl);
    web_PO.waitOffers(apiWaitTimeout);
    web_PO.waitForLoad('div', 'table-component-container-table', 'table');
    cy.get(".table-component-container-table").find('[mattooltip="Synchronize Offers"]').click();
    cy.get('.synchronize-window ').then((window) => {
      cy.wrap(window).should('contain','Selected region')
    })
  });
  ...
  it('should select "Acquisition" offerType', () => {
  ...
  it('should select "Single Code" offerCodeType', () => {
