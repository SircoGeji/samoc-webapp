...
export class RokuCampaignGridComponent
  ...
  ngOnInit(): void {
    combineLatest([this.rokuService.store, this.rokuService.product])
  ...
  startUp(): void {
    ...
    const selectedTab = JSON.parse(
      localStorage.getItem('rokuCampaignsTab') as string,
    );
    if (selectedTab) {
  ...
  setCampaignTableData() {
    this.campaignModuleSubscription = this.rokuService
      .getAllCampaigns()
      .pipe(takeUntil(this.destroy$))
      .subscribe((res: any) => {
        if (res && res.data) {
  ...
  setCampaignData(data: any[], defaultCountries: Set<string>): void {
    ...
    this.campaignsModuleData = data.map((module) => {
      const statusTitle: string = this.rokuManagementUtils.getStatusTitle(
        module.status,
      );
      let environments: string = '';
      if (module.status === ModuleStatus.LIVE && !!module.deployedTo) {
        environments = ` - ${this.rokuManagementUtils.getModuleDeploymentString(
          module.deployedTo,
        )}`;
      }
      ...
      if (!!module.endedOn && module.status === ModuleStatus.LIVE) {
        endedOn = `Ended - ${this.rokuManagementUtils.getModuleDeploymentString(
          module.endedOn,
        )}`;
      }
  ...
  setCampaignHistoryTableData() {
    this.campaignHistoryModuleSubscription = this.rokuService
  ...
  menuAction(type, element) {
    switch (type) {
      case 'DUPLICATE':
        if (this.selectedTab.value === 1) {
          this.router.navigate(
            ['/roku/campaigns/duplicate', element.campaignHistoryId],
            { queryParams: { isHistory: 'true' } },
          );
        } else {
          this.router.navigate([
            '/roku/campaigns/duplicate',
            element.campaignId,
          ]);
        }
        break;
      ...
      case 'VIEW':
        if (this.selectedTab.value === 1) {
          this.router.navigate(
            ['/roku/campaigns/view', element.campaignHistoryId],
            { queryParams: { isHistory: 'true' } },
          );
        } else {
          this.router.navigate(['/roku/campaigns/view', element.campaignId]);
        }
        break;
      case 'PUBLISH':
        const publishAction = {
          ...
          if (result) {
            switch (type) {
              ...
              case 'PUBLISH':
                ...
                element.statusTitle = `${this.rokuManagementUtils.getStatusTitle(
                  ModuleStatus.PUBLISH_PROGRESS,
                )} - ${this.rokuManagementUtils.getModuleDeploymentString(
                  result,
                )}`;
  ...
  getEditHRef(element) {
    if (this.selectedTab.value === 0) {
      return `/#/roku/campaigns/view/${element.campaignId}`;
    } else if (this.selectedTab.value === 1) {
      return `/#/roku/campaigns/view/${element.campaignHistoryId}?isHistory=true`;
    }
  }
  ...
  changeTab(event): void {
    ...
    localStorage.setItem('rokuCampaignsTab', JSON.stringify(event));
  }
