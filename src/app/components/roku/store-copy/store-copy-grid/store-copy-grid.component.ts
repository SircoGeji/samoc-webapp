...
export class RokuStoreCopyGridComponent
  ...
  setLanguages() {
    this.loaderService.show();
    this.languagesSubscription = this.rokuService
      .getLanguages()
      .pipe(takeUntil(this.destroy$))
      .subscribe((res: any) => {
  ...
  setTableData() {
    this.modulePackageSubscription = this.rokuService
      .getAllStoreCopies()
      .pipe(takeUntil(this.destroy$))
      .subscribe((res: any) => {
  ...
  menuAction(type, element) {
    switch (type) {
      ...
      case 'DEFAULT':
            ...
            this.modulePackages.forEach((elem) => {
              if (elem.isDefault && elem.storeCopyId !== element.storeCopyId) {
                this.rokuService
                  ...
                  .subscribe(() => {
                    this.loaderService.hide();
                    this.rokuManagementUtils.refreshTablePageNavigation(
                      this.router,
                    );
                  });
              }
        ...
        break;
      case 'VIEW':
        this.router.navigate(['/roku/store-copy/view', element.storeCopyId]);
        break;
      case 'PUBLISH':
        const action = {
  ...
  getEditHRef(element): string {
    return `/#/roku/store-copy/view/${element.storeCopyId}`;
  }
  ...
  createNewCopy() {
    if (this.defaultModule?.storeCopyId) {
      ...
      this.router.navigate(['/roku/store-copy/create']);
    }
  }
