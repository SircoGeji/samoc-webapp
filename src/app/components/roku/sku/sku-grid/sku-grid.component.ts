...
export class RokuSkuGridComponent
  ...
  setSKUTableData(): void {
    this.skuPackagesSubscription = this.rokuService
      .getAllSkus()
      .pipe(takeUntil(this.destroy$))
      .subscribe((res: any) => {
        if (res && res.data) {
  ...
  menuAction(type, element) {
    switch (type) {
      case 'DUPLICATE-SKU':
        ...
        this.router.navigate(['/roku/sku/duplicate', element.skuId]);
        break;
      case 'VIEW-SKU':
        this.router.navigate(['/roku/sku/view', element.skuId]);
        break;
      case 'PROMOTE-SKU':
        const promoteAction = {
          message: `${PROCEED_MESSAGE}PROMOTE "${element.skuName}" Sku module?`,
          action: 'prompt',
        };
  ...
  pullPromotionSkuAction(element) {
    ...
    const action = {
      message: `Do you wish to ACCEPT or DISCARD changes made in "${promotionSkuModule.name}" Client Dev Sku module?`,
  ...
  createNewSKU() {
    this.router.navigate(['/roku/sku/create']);
  }
          ...
          if (result) {
            switch (type) {
              ...
              case 'PUBLISH-SKU':
                ...
                break;
              case 'PROMOTE-SKU':
                this.promoteSkuModule(element.skuId);
                break;
              case 'PULL-PROMOTION-SKU':
                this.pullPromotionSku(element.skuId, result);
                break;
            }
          }
        });
  ...
  async promoteSkuModule(skuId: number) {
    try {
      this.loaderService.show();
      const response = await this.rokuService.promoteSku(skuId).toPromise();
  ...
  async pullPromotionSku(skuId: number, acceptChanges: string) {
    try {
      this.loaderService.show();
      const response = await this.rokuService
        .pullPromotionSku(skuId, acceptChanges === 'true')
        .toPromise();
      this.openResponseDialog(response);
    } catch (err) {
      this.openErrorDialog(err);
  ...
  async publishSkuModule(payload) {
    try {
      this.loaderService.show();
      const response = await this.rokuService.publishSku(payload).toPromise();
  ...
  getEditHRef(element) {
    return `/#/roku/sku/view/${element.skuId}`;
  }
  ...
  publishList() {
    this.openActionDialog('PUBLISH-SKU', null, {
      message: `${PROCEED_MESSAGE}PUBLISH all Sku modules?`,
  ...
  isPromotedModuleExist(element): boolean {
    const foundPromotedAppCopyModule = this.allSkuModuleData.find(
      (module) => module.promotionId === element.skuId,
    );
