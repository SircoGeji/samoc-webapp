...
export class RokuImagesGridComponent
  ...
  startUp() {
    ...
    const selectedTab = JSON.parse(
      localStorage.getItem('rokuImagesTab') as string,
    );
    if (selectedTab) {
    ...
    switch (type) {
      case 'DUPLICATE':
        this.router.navigate([
          '/roku/images/collection/duplicate',
          element.imageCollectionId,
        ]);
        break;
      case 'VIEW':
        this.router.navigate([
          '/roku/images/collection/view',
          element.imageCollectionId,
        ]);
        break;
      case 'PUBLISH':
  ...
  createNewBundle() {
    if (this.defaultBundle?.imageCollectionId) {
      ...
      this.router.navigate(['/roku/images/collection/create']);
    }
  }
  ...
  changeTab(event): void {
    ...
    localStorage.setItem('rokuImagesTab', JSON.stringify(event));
  }
          ...
          if (result) {
            switch (type) {
              ...
              case 'PUBLISH':
                ...
                this.publishModule(element.imageCollectionId, result);
                break;
            }
          }
        });
  ...
  getEditHRef(element) {
    if (this.selectedTab.value === 0) {
      return `/#/roku/images/collection/view/${element.imageCollectionId}`;
    } else if (this.selectedTab.value === 1) {
      return `/#/roku/images/gallery/view/${element.imageId}`;
    }
  }
