...
export class AndroidAppCopyGridComponent
  ...
  pullPromotionAction(element) {
    ...
    const action = {
      message: `Do you wish to ACCEPT or DISCARD changes made in "${promotionModule.appCopyName}" Client Dev AppCopy module?`,
  ...
  menuAction(type, element) {
    switch (type) {
      ...
      case 'DEFAULT':
        ...
        break;
      case 'VIEW':
        this.router.navigate(['/android/app-copy/view', element.appCopyId]);
        break;
      case 'PROMOTE-QA':
        const promoteQAAction = {
          message: `${PROCEED_MESSAGE}PROMOTE to QA "${element.appCopyName}" AppCopy module?`,
          action: 'prompt',
      ...
      case 'PROMOTE-PROD':
        const promoteProdAction = {
          message: `${PROCEED_MESSAGE}PROMOTE to Production "${element.appCopyName}" AppCopy module?`,
          action: 'prompt',
  ...
  createNewCopy() {
    if (!!this.getDefaultModule()) {
      this.router.navigate(
        ['/android/app-copy/duplicate', this.getDefaultModule().appCopyId],
        { queryParams: { create: true } },
      );
    } else {
      this.router.navigate(['/android/app-copy/create']);
    }
  }
        ...
        .subscribe((result) => {
          if (result) {
            switch (type) {
              ...
              case 'PROMOTE-QA':
              case 'PROMOTE-PROD':
                this.promoteAppCopyModule(
                  element.appCopyId,
                  receivedAction.promoteEnv,
                );
                break;
              case 'PULL-PROMOTION':
                this.pullPromotionAppCopy(
                  element.appCopyId,
                  receivedAction.promoteEnv,
                  result,
                );
  ...
  async promoteAppCopyModule(appCopyId: number, env: string) {
    try {
      this.loaderService.show();
      const response = await this.androidService
        .promoteAppCopy(appCopyId, env)
        .toPromise();
      this.openResponseDialog(response);
    } catch (err) {
      this.openErrorDialog(err);
  ...
  async pullPromotionAppCopy(
    ...
    try {
      this.loaderService.show();
      const response = await this.androidService
        .pullPromotionAppCopy(appCopyId, env, acceptChanges === 'true')
        .toPromise();
      this.openResponseDialog(response);
    } catch (err) {
      this.openErrorDialog(err);
  ...
  getEditHRef(element): string {
    return `/#/android/app-copy/view/${element.appCopyId}`;
  }
  ...
  isOriginalModuleExist(element): boolean {
    const foundOriginalAppCopyModule = this.allAppCopyModuleData.find(
      (module) =>
        module.appCopyId === element.promotionId &&
  ...
  isPromotedModuleExist(element): boolean {
    const foundPromotedAppCopyModule = this.allAppCopyModuleData.find(
      (module) =>
        module.promotionId === element.appCopyId &&
