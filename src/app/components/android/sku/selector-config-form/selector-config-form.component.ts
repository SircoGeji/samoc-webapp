...
export class SelectorConfigFormComponent
  ...
  ngOnInit(): void {
    ...
    forkJoin([
      ...
      .subscribe((resultArray) => {
        const [selectorConfigsRes, regionsRes, skusRes] = resultArray;
        if (!!selectorConfigsRes.data && !!selectorConfigsRes.data.length) {
          this.allSelectorConfigModules = selectorConfigsRes.data.filter(
            (el) => {
              return (
      ...
      if (
        ...
        action = {
          message:
            `${
              this.importedModuleData.status === ModuleStatus.LIVE
                ? 'This is Live SELECTOR CONFIG, after update it will be published automatically. '
                : ''
            }` + `${PROCEED_MESSAGE} UPDATE ALL?`,
          action: 'prompt',
        };
  ...
  publishAfterUpdate(res) {
    ...
    this.androidFormsService
      .getPublishRequests(
        ...
        this.moduleId,
        this.androidService.publishSelectorConfig,
      )
      .subscribe(() => {
        this.androidFormsService.navigateBackOrOpenDialog(
          this.openResponseDialog,
      ...
      this.openActionDialog(action).subscribe((res) => {
        switch (res) {
          ...
          case 'leave':
            ...
            this.androidFormsService.navigateBack('sku/selector-config');
            break;
          case 'cancel':
            this.subject.next(false);
            this.androidFormsService.setProperHistory(
