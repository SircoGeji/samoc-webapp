...
export class RegionsLanguagesGridComponent
  ...
  @ViewChild('androidRegionTable', { read: MatSort, static: true })
  regionsSort: MatSort;
  ...
  ngOnInit(): void {
    ...
    const selectedTab = JSON.parse(
      localStorage.getItem('androidRegionTab') as string,
    );
    if (selectedTab) {
  ...
  changeTab(event): void {
    ...
    localStorage.setItem('androidRegionTab', JSON.stringify(event));
  }
  ...
  createRegion(): void {
    this.dialog.open(RegionModalComponent, {
      width: '385px',
      height: '615px',
